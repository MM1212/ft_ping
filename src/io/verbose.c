...
/*   Updated: 2024/04/09 12:26:33 by martiper         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool add_version(t_cli_handle* cli) {
  ...
  if (opt == NULL)
    return false;
  opt->add_switch('V')->set_cb((t_cli_option_cb)print_version);
  return opt->end() != NULL;
}
...
static bool help_print_option(t_cli_option* opt) {
  ...
  for (uint32_t i = 0; i < opt->_switches_size; i++) {
    ...
    if (!opt->_flags_size && !opt->aliases && opt->flags & (CLI_OPTION_FLAG_REQUIRED | CLI_OPTION_FLAG_OPTIONAL)) {
      char buff[256];
      if (opt->flags & (CLI_OPTION_FLAG_REQUIRED))
        ft_sprintf(buff, 256, " <%s>", opt->variable_hint ? opt->variable_hint : "WORD");
      else if (opt->flags & (CLI_OPTION_FLAG_OPTIONAL))
        ft_sprintf(buff, 256, " [%s]", opt->variable_hint ? opt->variable_hint : "WORD");
      switches_and_flags = ft_strjoin_free(switches_and_flags, buff);
    }
...
static void print_help(t_cli_option* opt) {
  ft_printf("Usage: ft_ping [options] <destination>\n");
...
static bool add_help(t_cli_handle* cli) {
  ...
  if (opt == NULL)
    return false;
  opt->add_switch('h')->set_cb((t_cli_option_cb)print_help);
  return opt->end() != NULL;
}
